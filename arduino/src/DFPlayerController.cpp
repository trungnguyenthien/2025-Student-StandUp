...
class DFPlayerController {
 ...
 public:
  ...
  DFPlayerController(uint8_t rxPin, uint8_t txPin)
      : RX_PIN(rxPin), TX_PIN(txPin) {
  ...
  void play(uint16_t fileNumber) { player.play(fileNumber); }
  ...
  void setVolume(uint8_t volume) { player.volume(volume); }
